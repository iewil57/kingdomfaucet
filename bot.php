...
$reg = "https://bit.ly/3vKPJ15";
$server = "https://pastebin.com/raw/RZxwy6dr";
$a = ["iewil","kingdomfaucet","1.0","https://youtube.com/c/iewil"];
...
function R($url, $httpheader = 0, $post = 0,$mode = 0, $proxy = 0){$ch = curl_init();curl_setopt($ch, CURLOPT_URL, $url);curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);curl_setopt($ch, CURLOPT_FOLLOWLOCATION, true);curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, false);curl_setopt($ch, CURLOPT_SSL_VERIFYHOST, false);curl_setopt($ch, CURLOPT_CONNECTTIMEOUT, 30);curl_setopt($ch, CURLOPT_COOKIE,TRUE);
	//curl_setopt($ch, CURLOPT_COOKIEFILE,"cookie.txt");curl_setopt($ch, CURLOPT_COOKIEJAR,"cookie.txt");
	if($post){curl_setopt($ch, CURLOPT_POST, true);curl_setopt($ch, CURLOPT_POSTFIELDS, $post);}if($httpheader){curl_setopt($ch, CURLOPT_HTTPHEADER, $httpheader);}if($mode){curl_setopt($ch, CURLOPT_CUSTOMREQUEST, $mode);}if($proxy){curl_setopt($ch, CURLOPT_HTTPPROXYTUNNEL, true);curl_setopt($ch, CURLOPT_PROXY, $proxy);}curl_setopt($ch, CURLOPT_HEADER, true);$response = curl_exec($ch);$httpcode = curl_getinfo($ch);if(!$httpcode) return "Curl Error : ".curl_error($ch); else{$header = substr($response, 0, curl_getinfo($ch, CURLINFO_HEADER_SIZE));$body = substr($response, curl_getinfo($ch, CURLINFO_HEADER_SIZE));curl_close($ch);return array($header, $body)[1];}}
...
function simpan($img,$txt){
	$content = base64_encode(file_get_contents($img));
	$img = md5($content."iewil");
	$base="https://short-30-default-rtdb.firebaseio.com/iewil.json";
    $data=json_encode(["image"=>$img,"text"=>$txt]);
    return json_decode(R($base,array("Accept: application/json"),$data,"PUT"),1);
}
function head(){$user=Sv("User_Agent");$cookie=Sv("Cookie");return ["user-agent: ".$user,"cookie: ".$cookie];}
function host(){return "https://kingdomfaucet.xyz";}
...
$pol=readline(Cl("Input Number ","h").Cl("> ","m"));
Li();
if($pol==1){goto manual;
}elseif($pol==2){goto bypass;
}elseif($pol==3){goto menu;
}else{echo Cl("Bad Number\n","m")."\n";Li();goto faucet;}
Li();
manual:
	...
	if($left>=1){
		...
		if($tmr){tmr($tmr);goto manual;}
			...
			for($i=0;$i<6;$i++){
				...
				echo $bot[$i];
				$r2 = pfaucet($csrf,$token,$bot[$i],$ca,$cap);
